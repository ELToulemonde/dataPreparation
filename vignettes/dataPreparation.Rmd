...
setColAsNumeric             | generateFromCharacter    | fastRound           |              |
setColAsDate                |                          |                     |              |
setColAsFactor              |                          |                     |              |
...
## Identifing factor that shouldn't be
...
Another way to work around dates would be to aggregate them at some level. 
```{r}
...
Now that we have built some acceptable variables from those dates, we drop them:
```{r}
...
Finaly, to generate a description file from this data set, function `description` is available.

It will discribe, the set and it's variables. Here we put the result in a txt file:
```{r, tidy=TRUE, tidy.opts=list(length.cutoff=10)}
...
If you would like to add some features to this package, notice some issues, please tell us on [GitHub](https://github.com/ELToulemonde/dataPreparation/issues). Also if you want to contribute, please don't hesitate to contact us.
